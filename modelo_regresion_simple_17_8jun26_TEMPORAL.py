...
# ============================================================
# SPLIT TEMPORAL POR fe_carga_efectiva
# ============================================================

MIN_TEST_FILAS = 1000
MIN_TEST_POSITIVOS = 50
MIN_TEST_NEGATIVOS = 50


def temporal_train_test_split(df_model, target_col):
    """
    Test = última fe_carga_efectiva disponible.
    Si no hay suficientes datos/positivos/negativos, añade la penúltima, etc.
    Train = todas las cargas anteriores.
    """
    ...
    if df_model["fe_carga_efectiva_dt"].isna().all():
        raise ValueError("fe_carga_efectiva no se ha podido parsear.")

    fechas = sorted(df_model["fe_carga_efectiva_dt"].dropna().unique())

    fechas_test = []
    for fecha in reversed(fechas):
        fechas_test.append(fecha)
        mask_test = df_model["fe_carga_efectiva_dt"].isin(fechas_test)
        y_test_tmp = df_model.loc[mask_test, target_col]

        n_test = len(y_test_tmp)
        pos = int(y_test_tmp.sum())
        neg = int(n_test - pos)

        if (
            n_test >= MIN_TEST_FILAS
            and pos >= MIN_TEST_POSITIVOS
            and neg >= MIN_TEST_NEGATIVOS
        ):
            break
    ...
    for f in sorted(fechas_test):
        print("-", pd.Timestamp(f).date())

    print("\nRango TRAIN:")
    print(df_model.loc[mask_train, "fe_carga_efectiva_dt"].min(), "->", df_model.loc[mask_train, "fe_carga_efectiva_dt"].max())

    print("\nRango TEST:")
    print(df_model.loc[mask_test, "fe_carga_efectiva_dt"].min(), "->", df_model.loc[mask_test, "fe_carga_efectiva_dt"].max())

    print("\nFilas TRAIN:", mask_train.sum())
    print("Filas TEST:", mask_test.sum())
    print("Positivos TEST:", int(df_model.loc[mask_test, target_col].sum()))
    print("Negativos TEST:", int(mask_test.sum() - df_model.loc[mask_test, target_col].sum()))
    print("Tasa TEST:", df_model.loc[mask_test, target_col].mean())

    return mask_train, mask_test, sorted(fechas_test)
...
if TARGET not in df_raw.columns:
    raise ValueError(f"No existe {TARGET} en {ARCHIVO_IN}")
...
mask_train, mask_test, fechas_test = temporal_train_test_split(df_model, TARGET)
...
print("\n--- MÉTRICAS TEST TEMPORAL VENTA POST-CONTACTO ---")
...
tabla["lift_vs_media"] = tabla["tasa_venta"] / y_test.mean()

print("\n--- LIFT POR DECILES VENTA POST-CONTACTO - TEST TEMPORAL ---")
print(tabla)
tabla.to_csv("lift_deciles_venta_postcontacto_temporal.csv")

os.makedirs(OUT_FIG_DIR, exist_ok=True)

plt.figure(figsize=(9, 5))
plt.bar(tabla.index.astype(str), tabla["tasa_venta"])
plt.title("Tasa de venta post-contacto por decil - test temporal (1 = TOP)")
plt.xlabel("Decil")
plt.ylabel("Tasa de venta")
plt.tight_layout()
plt.savefig(os.path.join(OUT_FIG_DIR, "temporal_tasa_venta_postcontacto_por_decil.png"), dpi=200, bbox_inches="tight")
plt.show()
